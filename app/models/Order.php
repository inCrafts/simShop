...
class Order extends AppModel {
    ...
    public static function mailOrder($order_id, $user_email){
        ...
        // Create a message
        ob_start();
        require APP . '/views/Mail/orderMail.php';
        $mail = ob_get_clean();

        $message_client = (new Swift_Message("Вы совершили заказ №{$order_id} В магазине " . App::$app->getProperty('shop_name')))
            ->setFrom([App::$app->getProperty('smtp_login') => App::$app->getProperty('shop_name')])
            ->setTo($user_email)
            ->setBody($mail, 'text/html')
        ;

        $message_admin = (new Swift_Message("Ваш заказ №{$order_id}"))
            ->setFrom([App::$app->getProperty('smtp_login') => App::$app->getProperty('shop_name')])
